...
// Fix the commented components before re-enabling them in the docs catalog.
...
export function getComponentSlug(component: Pick<ComponentListItem, 'url'>): string {
  return component.url.replace('/docs/components/', '')
}

export function getComponentBySlug(slug: string): ComponentListItem | null {
  return components.find((component) => getComponentSlug(component) === slug) ?? null
}
