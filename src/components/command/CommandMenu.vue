...
import type { Color, ColorPalette } from '@/utils/colors'
import { useClipboard } from '@vueuse/core'
import { CornerDownLeft, Square, ArrowRight } from 'lucide-vue-next'
...
interface NavigationItem {
  title: string
  type: 'group' | 'page' | 'component'
  path: string
  stem?: string
  children?: {title: string, type: 'group' | 'page' | 'component', path: string, stem?: string}[]
}
  ...
  <Dialog v-model:open="open">
    <DialogTrigger as-child>
      <Button
        variant="outline"
        :class="cn(
          'bg-surface hover:bg-slate-200 text-foreground dark:bg-card relative h-8 w-full justify-start pl-3 font-medium shadow-none sm:pr-12',
        )"
        @click="open = true"
      >
        ...
        <CommandList class="no-scrollbar min-h-80 scroll-pt-2 scroll-pb-1.5">
          ...
          >
            <template v-if="group.type === 'group'">
              <!-- NavigationItem -->
              <CommandMenuItem
                v-for="item in group.children?.filter((i: any) => i.type === 'page' || i.type === 'component')" 
                :key="item.title"
